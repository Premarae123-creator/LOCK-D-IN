...
io.on('connection',s=>{
 ...
 s.on('start',({code,key},cb)=>{
  ...
  if(claimed(r).length<5)return cb&&cb({ok:false,msg:"At least 5 players must be LOCK'D IN before the game can start."});
  r.status='live';r.winner=null;push(r,'🚨 LOCKS CLOSED — WE ARE LIVE!','live');emit(r);cb&&cb({ok:true});
 });
 s.on('drop',({code,count,key},cb)=>{
  ...
  finishIfNeeded(r);emit(r);io.to(r.code).emit('dropEvent',{drops,remaining:active(r).length,winner:r.winner});cb&&cb({ok:true});
 });
